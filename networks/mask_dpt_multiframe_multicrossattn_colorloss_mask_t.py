...
class Masked_DPT_Multiframe_MultiCrossAttn_ColorLoss_mask_t(nn.Module):
    def __init__(
        self,
        *,
        encoder,
        ...
        self.scratch.output_conv_depth = head = nn.Sequential(
        ...
        # JINLOVESPHO
        self.scratch.output_conv_color = head = nn.Sequential(
            nn.Conv2d(256, 256 // 2, kernel_size=3, stride=1, padding=1),
            Interpolate(scale_factor=2, mode="bilinear", align_corners=True),
            nn.Conv2d(256 // 2, 32, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
    ...
    def forward(self, img_frames, K = 1, mode=None):
        ...
        if K == 1:
            ...
            glob1_layer_1 = self.encoder.transformer.features[0]      # 중간 layer output feature from 2nd layer
            glob1_layer_2 = self.encoder.transformer.features[1]      # 중간 layer output feature from 5th layer
            glob1_layer_3 = self.encoder.transformer.features[2]      # 중간 layer output feature from 8th layer
            glob1_layer_4 = self.encoder.transformer.features[3]      # 중간 layer output feature from 11th layer     # 이게 마지막 layer outputd 이어서 glob랑 똑같네. interesting
            ...
            cross_attn_out1 = self.cross_attn_module1(glob1_layer_1, glob2_layer_1) 
            cross_attn_out2 = self.cross_attn_module2(glob1_layer_2, glob2_layer_2)
            cross_attn_out3 = self.cross_attn_module3(glob1_layer_3, glob2_layer_3)
            cross_attn_out4 = self.cross_attn_module4(glob1_layer_4, glob2_layer_4)
            
            layer_1 = glob1_layer_1 + cross_attn_out1
            layer_2 = glob1_layer_2 + cross_attn_out2
            layer_3 = glob1_layer_3 + cross_attn_out3
            layer_4 = glob1_layer_4 + cross_attn_out4
        ...
        pred_depth = self.scratch.output_conv_depth(path_1) * self.max_depth
        pred_color = self.scratch.output_conv_color(path_1)
