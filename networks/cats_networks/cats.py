...
class TransformerAggregator(nn.Module):
    ...
    def forward(self, corr, src, tgt):
        ...
        breakpoint()
        x = torch.cat((x, src), dim=3) + pos_embed
...
class CATs(nn.Module):
    ...
    def forward(self, tgt, src):
        ...
        src_feats = self.feature_extraction(src)
        tgt_feats = self.feature_extraction(tgt)
        ...
        for i, (src, tgt) in enumerate(zip(src_feats, tgt_feats)):
            corr = self.corr(self.l2norm(src), self.l2norm(tgt))    # (b,256,256)
            corrs.append(corr)
            src_feats_proj.append(self.proj[i](src.flatten(2).transpose(-1, -2)))   # (b,256,128)
            tgt_feats_proj.append(self.proj[i](tgt.flatten(2).transpose(-1, -2)))
        
        src_feats = torch.stack(src_feats_proj, dim=1)  # (b,8,n1,d) (b,8,256,128)
