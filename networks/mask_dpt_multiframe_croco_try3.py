...
class Masked_DPT_Multiframe_Croco_Try3(nn.Module):
    def __init__(
        self,
        *,
        encoder,
        ...
        # cat linear 
        self.cat_linear1 = nn.Linear(6528, 768)
        self.cat_linear2 = nn.Linear(6528, 768)
        self.cat_linear3 = nn.Linear(6528, 768)
        self.cat_linear4 = nn.Linear(6528, 768)
    ...
    def forward(self, img_frames, K = 1, mode=None):
        ...
        avg_cmap2=torch.zeros(c_attn_map2[0].shape).to(self.device)
        avg_cmap3=torch.zeros(c_attn_map3[0].shape).to(self.device)
        avg_cmap4=torch.zeros(c_attn_map4[0].shape).to(self.device)
        ...
        # TRY3-2 위에 주석하고 여기만 추가 
        cmap1 = avg_cmap1.permute(0,2,1,3)  # (b,n,head,n)
        cmap2 = avg_cmap2.permute(0,2,1,3)
        cmap3 = avg_cmap3.permute(0,2,1,3)
        cmap4 = avg_cmap4.permute(0,2,1,3)
        ...
        # TRY3-1 and 3-2
        cat_out_and_map1 = torch.cat([glob1_layer_1,cmap1], dim=-1)
        cat_out_and_map2 = torch.cat([glob1_layer_2,cmap2], dim=-1)
        cat_out_and_map3 = torch.cat([glob1_layer_3,cmap3], dim=-1)
        cat_out_and_map4 = torch.cat([glob1_layer_4,cmap4], dim=-1)
        
        layer_1 = self.cat_linear1(cat_out_and_map1)
        layer_2 = self.cat_linear2(cat_out_and_map2)
        layer_3 = self.cat_linear3(cat_out_and_map3)
        layer_4 = self.cat_linear4(cat_out_and_map4)
