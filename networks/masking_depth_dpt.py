...
class Masked_DPT_Multiframe_Croco_Baseline(nn.Module):
    def __init__(
        self,
        *,
        encoder,
        ...
        # get_image_size()= (192,640)
        # get_patch_size()= (16,16)
        self.unflatten = nn.Sequential(
            nn.Unflatten( 2, torch.Size([ encoder.get_image_size()[0] // encoder.get_patch_size()[0], encoder.get_image_size()[1] // encoder.get_patch_size()[1]]))
        )
        ...
        self.cross_attn_module1 = CrossAttention_Module(ca_dim=vit_features, ca_num_heads=12, ca_depth=cross_attn_depth, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, norm_mem=True, rope=self.rope)
        self.cross_attn_module2 = CrossAttention_Module(ca_dim=vit_features, ca_num_heads=12, ca_depth=cross_attn_depth, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, norm_mem=True, rope=self.rope)
        self.cross_attn_module3 = CrossAttention_Module(ca_dim=vit_features, ca_num_heads=12, ca_depth=cross_attn_depth, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, norm_mem=True, rope=self.rope)
        self.cross_attn_module4 = CrossAttention_Module(ca_dim=vit_features, ca_num_heads=12, ca_depth=cross_attn_depth, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, norm_mem=True, rope=self.rope)
    ...
    def forward(self, inputs_dic, K = 1, mode=None):
        ...
        img_frames = [inputs_dic['color',0,0], inputs_dic['color',-1,0]]
        ...
        # tokenized_frames[1] : prev t-1 frame (b,480,768)
        tokenized_frames = []   
        poses = []
        ...
        # ca_map1 = torch.cat(c_attn_map1, dim=1)     # concat head dim
        # ca_map2 = torch.cat(c_attn_map2, dim=1)     # (b,head,n,n)
        # ca_map3 = torch.cat(c_attn_map3, dim=1)
        # ca_map4 = torch.cat(c_attn_map4, dim=1)
        
        # ca_map1 = ca_map1.mean(dim=1)   # (b,n,n)
        # ca_map2 = ca_map2.mean(dim=1)
        # ca_map3 = ca_map3.mean(dim=1)
        # ca_map4 = ca_map4.mean(dim=1)
        ...
        return outputs#, features, fusion_features
