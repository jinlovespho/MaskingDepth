...
class Masked_DPT_Multiframe_Croco_Try5(nn.Module):
    def __init__(
        ...
        *,
        ...
        # conv4d 
        self.conv4d_module1 = nn.Sequential( Conv4d_Module(in_c=self.encoder.heads, out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)),
                                             Conv4d_Module(in_c=128,                out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)),
                                             Conv4d_Module(in_c=128,                out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)), )
        
        self.conv4d_module2 = nn.Sequential( Conv4d_Module(in_c=self.encoder.heads, out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)),
                                             Conv4d_Module(in_c=128,                out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)),
                                             Conv4d_Module(in_c=128,                out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)), )
        
        self.conv4d_module3 = nn.Sequential( Conv4d_Module(in_c=self.encoder.heads, out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)),
                                             Conv4d_Module(in_c=128,                out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)),
                                             Conv4d_Module(in_c=128,                out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)), )
        
        self.conv4d_module4 = nn.Sequential( Conv4d_Module(in_c=self.encoder.heads, out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)),
                                             Conv4d_Module(in_c=128,                out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)),
                                             Conv4d_Module(in_c=128,                out_c=128, ks=(3,3,3,3), pd=(1,1,0,0), str=(1,1,1,1)), )
        ...
        # cmap linear 
        self.cmap1_linear = nn.Linear(128, vit_features)    # (128,768)
        self.cmap2_linear = nn.Linear(128, vit_features)  
        self.cmap3_linear = nn.Linear(128, vit_features)  
        self.cmap4_linear = nn.Linear(128, vit_features)  
