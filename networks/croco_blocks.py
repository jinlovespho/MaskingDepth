...
class CrossAttention_Module(nn.Module):
    ...
    def __init__(self, ca_dim, ca_num_heads, ca_depth, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, norm_mem=True, rope=None):
        super().__init__()
        self.cross_attn_blks = nn.ModuleList([ CrossAttention_Block(ca_dim, ca_num_heads, mlp_ratio, qkv_bias, drop, attn_drop,
...
# x1 = torch.rand(8,480,768)
# y1 = torch.rand(8,480,768)

# db = DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=rope)
# out1 = db(x1)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # breakpoint()                                                   # x.shape (B, 3, 224, 224)
        x = self.proj(x)                                                # x.shape (B, 768, 14,14)
        pos = self.position_getter(B, x.size(2), x.size(3), x.device)   # pos.shape (B, 196,2)
        if self.flatten:
