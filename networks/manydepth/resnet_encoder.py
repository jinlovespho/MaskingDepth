...
from networks.manydepth.layers import BackprojectDepth, Project3D
...
class ResnetEncoderMatching(nn.Module):
    ...
    def match_features(self, current_feats, lookup_feats, relative_poses, K, invK):
        ...
        for batch_idx in range(len(current_feats)):
            ...
            world_points = self.backprojector(self.warp_depths, _invK)  # (96,4,7680), 7680 = 48 * 160
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                diffs = torch.abs(warped - current_feats[batch_idx:batch_idx + 1]).mean(1) * edge_mask  # (96,48,160)
                # warped: (96,64,48,160)
                # current_feats: ()
    ...
    def feature_extraction(self, image, return_all_feats=False):
        ...
        image = (image - 0.45) / 0.225  # imagenet normalisation     (b,3,192,640)
        feats_0 = self.layer0(image)                                # (b,64,96,320)
        feats_1 = self.layer1(feats_0)                              # (b,64,48,160) 
    ...
    def forward(self, current_image, lookup_images, poses, K, invK,
        ...
        current_feats = self.features[-1]   # current_image 가 feature extraction 거친 output   (b,64,48,160)

        # feature extraction on lookup images - disable gradients to save memory
        with torch.no_grad():
            if self.adaptive_bins: # True
                self.compute_depth_bins(min_depth_bin, max_depth_bin)   # 여기서 self.warp_depths 가 채워지네 
            ...
            lookup_feats = self.feature_extraction(lookup_images, return_all_feats=False)   # (b,64,48,160)
            
            # feature extraction 된 t frame 과 t-1 frame 은 각각 current_feats 과 lookup_feats 이다.
            # 둘 다 shape: (b,64,48,160)
            ...
            cost_volume, missing_mask = self.match_features(current_feats, lookup_feats, poses, K, invK)    # (b,96,48,160)
            confidence_mask = self.compute_confidence_mask(cost_volume.detach() * (1 - missing_mask.detach()))  # (b,96,48,160)
        ...
        curr_prev_img = torch.cat([current_image, lookup_images], dim=2)
        save_image(curr_prev_img, 'curr_prev_img.jpg')
        save_image(current_image, 'current_img.jpg')
        save_image(lowest_cost, 'vis_cost.jpg')
        save_image(lowest_cost, 'vis_cost_n.jpg', normalize=True)
        print(poses)
        print(K)
        print(invK)
        # print(self.warp_depths)
        # breakpoint()
        
        # mask the cost volume based on the confidence
        cost_volume *= confidence_mask.unsqueeze(1)
        post_matching_feats = self.reduce_conv(torch.cat([self.features[-1], cost_volume], 1))  # cost_volume 과 target feature map 을 concat 해서 넣네!
...
class ResnetEncoder(nn.Module):
    ...
    def forward(self, input_image):
        ...
        # 사이 intermediate feature 들도 담아서 전부 return 하는 것. 
        # 이 중에서는 사실 마지막 feature 인 self.features[-1] 만 필요
