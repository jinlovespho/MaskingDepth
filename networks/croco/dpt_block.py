...
class DPTOutputAggregateAdapter(nn.Module):
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], image_size, attn_map,intrinsics=None):
        ...
        # Hook decoder onto 4 layers from specified ViT layers
        layers = [encoder_tokens[hook] for hook in self.hooks]  # b n d (b 480 768)
        if self.residual:
            layers = [layers[0]+layers[4], layers[1]+layers[5], layers[2]+layers[6], layers[3]+layers[7]]
        attn_maps = [(attn_map[hook-12] + attn_map[hook-13] + attn_map[hook-14])/3. for hook in self.hooks] # b h n n (b 12 480 480)

        # Extract only task-relevant tokens and ignore global tokens.
        layers = [self.adapt_tokens(l) for l in layers]
        # Reshape tokens to spatial representation
        layers = [rearrange(l, 'b (nh nw) c -> b c nh nw', nh=N_H, nw=N_W) for l in layers] # b n d -> b 768 12 40
