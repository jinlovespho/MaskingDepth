...
class DPTOutputAdapter_MF_Sup_Try1(nn.Module):
    """DPT output adapter.
    ...
    def forward(self, enc_dec_tkns: List[torch.Tensor], image_size):
        ''' enc_dec_tkns contains 12(enc)+12(dec) tkns
            enc_dec_tkns[0~11] -> enc
            enc_dec_tkns[12~23] -> dec 
        '''
        ...
        layers = [enc_dec_tkns[hook] for hook in self.hooks]    # self.hooks=[11,15,19,23]

        # Reshape tokens to spatial representation
        # b n d -> b c h w 
        layers = [rearrange(l, 'b (nh nw) c -> b c nh nw', nh=N_H, nw=N_W) for l in layers] # b 768 12 40

        # act_postprocess upsamples spatial resolution to various sizes
        layers = [self.act_postprocess[idx](l) for idx, l in enumerate(layers)]
        '''
        layers[0] b 96 48 160
        layers[1] b 192 24 80
        layers[2] b 384 12 40
        layers[3] b 768 6 20
        '''
        
        # scratch.layer_rn makes all channel size to 256 while maintaining spatial resolution
        layers = [self.scratch.layer_rn[idx](l) for idx, l in enumerate(layers)]
        '''
        layers[0] b 256 48 160
        layers[1] b 256 24 80
        layers[2] b 256 12 40
        layers[3] b 256 6 20
        '''

        # Fuse layers using refinement stages
        path_4 = self.scratch.refinenet4(layers[3])             # b 256 12 40
        path_3 = self.scratch.refinenet3(path_4, layers[2])     # b 256 24 80
        path_2 = self.scratch.refinenet2(path_3, layers[1])     # b 256 48 160
        path_1 = self.scratch.refinenet1(path_2, layers[0])     # b 256 96 320
