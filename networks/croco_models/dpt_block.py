...
class DPTOutputAdapter(nn.Module):
    ...
    def adapt_tokens(self, enc_tkns):
        # Adapt tokens
        x = []
        x.append(enc_tkns[:, :])
        x = torch.cat(x, dim=-1)
    ...
    def forward(self, enc_tkns: List[torch.Tensor], image_size):
        
        # enc_tkns 에 encoder 24 features 와 decoder 12 features list 로 같이 담겨 O
        # breakpoint()
        ...
        # self.hooks=[23(enc),27(dec),31(dec),35(dec)]
        # enc_tkns = 24(enc) + 12(dec) 

        # Hook decoder onto 4 layers from specified ViT layers
        layers = [enc_tkns[hook] for hook in self.hooks]
        ...
        path_4 = self.scratch.refinenet4(layers[3])             # (b,256,22,44)
        path_3 = self.scratch.refinenet3(path_4, layers[2])     # (b,256,44,88)
        path_2 = self.scratch.refinenet2(path_3, layers[1])     # (b,256,88,176)
        path_1 = self.scratch.refinenet1(path_2, layers[0])     # (b,256,172,352)
