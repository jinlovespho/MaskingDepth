...
import networks.manydepth_networks
...
def model_load(train_args, device):
    ...
    elif train_args.model_info == 'cs_manydepth':
        
        encoder_path = os.path.join(train_args.ckpt_path, "encoder.pth")
        decoder_path = os.path.join(train_args.ckpt_path, "depth.pth")
        encoder_class = networks.manydepth_networks.ResnetEncoderMatching
        
        encoder_dict = torch.load(encoder_path)
        encoder_opts = dict(num_layers=18,  # default
                                pretrained=False,
                                input_width=encoder_dict['width'],
                                input_height=encoder_dict['height'],
                                adaptive_bins=True,
                                min_depth_bin=0.1, max_depth_bin=20.0,
                                depth_binning='linear', # default
                                num_depth_bins=96) # default
        
        model['encoder'] = encoder_class(**encoder_opts)
        model['depth'] = networks.DepthDecoder(model['encoder'].num_ch_enc)

        model_dict = model['encoder'].state_dict()
        
        pose_enc_dict = torch.load(os.path.join(train_args.ckpt_path, "pose_encoder.pth"))
        pose_dec_dict = torch.load(os.path.join(train_args.ckpt_path, "pose.pth"))

        model['pose_encoder'] = networks.manydepth_networks.ResnetEncoder(18, False, num_input_images=2)
        model['pose_decoder'] = networks.manydepth_networks.PoseDecoder(model['pose_encoder'].num_ch_enc, num_input_features=1, num_frames_to_predict_for=2)

        msg1=model['encoder'].load_state_dict({k: v for k, v in encoder_dict.items() if k in model_dict})
        msg2=model['depth'].load_state_dict(torch.load(decoder_path))
        msg3=model['pose_encoder'].load_state_dict(pose_enc_dict, strict=True)
        msg4=model['pose_decoder'].load_state_dict(pose_dec_dict, strict=True)
        print(msg1)
        print(msg2)
        print(msg3)
        print(msg4)
