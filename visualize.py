    ...
    with open(args.conf, 'r') as f:
        ...
        for epoch in range(1):
            ...
            with torch.no_grad():
                ...
                ca2_maps=[]
                for name, module in model['depth'].module.named_modules():
                    if 'cross_attn_module2' in name and 'c_attn_drop' in name:
                        ca2_names.append(name)
                        ca2_modules.append(module)
                        module.register_forward_hook(lambda m,i,o: ca2_maps.append(o.detach().cpu()) )
                        
                ca3_names=[]
                ca3_modules=[]
                ca3_maps=[]
                for name, module in model['depth'].module.named_modules():
                    if 'cross_attn_module3' in name and 'c_attn_drop' in name:
                        ca3_names.append(name)
                        ca3_modules.append(module)
                        module.register_forward_hook(lambda m,i,o: ca3_maps.append(o.detach().cpu()) )
                ...
                ca4_maps=[]
                for name, module in model['depth'].module.named_modules():
                    if 'cross_attn_module4' in name and 'c_attn_drop' in name:
                        ca4_names.append(name)
                        ca4_modules.append(module)
                        module.register_forward_hook(lambda m,i,o: ca4_maps.append(o.detach().cpu()) )
                ...
                for i, inputs in enumerate(tqdm(val_loader)):
                    ...
                    if train_cfg.data.dataset=='kitti_depth_multiframe':
                        ...
                        for vis_idx in to_vis_idx:
                            pho_visualize_cross_attn_map( ca_module_num=1, iter_num=i, curr_frame=inputs[0]['color'], prev_frame=inputs[1]['color'], ca_maps=maps1, discard_ratio=discard_ratio, vis_idx=vis_idx)
                            pho_visualize_cross_attn_map( ca_module_num=2, iter_num=i, curr_frame=inputs[0]['color'], prev_frame=inputs[1]['color'], ca_maps=maps2, discard_ratio=discard_ratio, vis_idx=vis_idx)
                            pho_visualize_cross_attn_map( ca_module_num=3, iter_num=i, curr_frame=inputs[0]['color'], prev_frame=inputs[1]['color'], ca_maps=maps3, discard_ratio=discard_ratio, vis_idx=vis_idx)
                            pho_visualize_cross_attn_map( ca_module_num=4, iter_num=i, curr_frame=inputs[0]['color'], prev_frame=inputs[1]['color'], ca_maps=maps4, discard_ratio=discard_ratio, vis_idx=vis_idx)
